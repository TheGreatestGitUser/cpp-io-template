...
//#define nextInteger(x) do{ bool neg = false; register int c; x = 0; c = getchar_unlocked(); if (c=='-') { neg = true; c = getchar_unlocked(); } for (; (c>47 && c<58); c=getchar_unlocked()) { x = x * 10 + c - 48; } if (neg){ x *= -1; } } while (0) // This one is used for integers
